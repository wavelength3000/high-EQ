...
object AiClient {
    ...
    /**
     * 把勾选风格对应的内置素材拼进 prompt。重油素材（土味情话/舔狗自嘲）
     * 由 ReplyBank 按油腻度过滤，不到「微甜」根本不进来。
     * 素材是工具箱不是稿子：模型只在贴切时借用或化用，不合适就自己写。
     */
    private fun materialBlock(tones: List<ToneSpec>, greasiness: Int): String {
        val sb = StringBuilder()
        tones.forEach { t ->
            val items = ReplyBank.byTone(t.id, greasiness)
    ...
    /**
     * 按对方最近说的话匹配出的场景素材。排在最前面，让模型优先从这里挑。
     */
    private fun scenarioBlock(scenarios: List<ReplyBank.Scenario>): String {
        if (scenarios.isEmpty()) return ""
        val sb = StringBuilder("对方刚说到的场景，下面的段子最贴，优先从里面挑：\n")
        scenarios.forEach { s ->
            sb.append("【场景·").append(s.label).append("】\n")
            sb.append(s.lines.joinToString("\n") { "· $it" })
            sb.append("\n\n")
        }
        return sb.toString().trim()
    }

    private fun systemPrompt(tones: List<ToneSpec>, scenarios: List<ReplyBank.Scenario>): String {
...
""".trim()
        ...
        val bank = if (Prefs.useReplyBank)
            listOf(scenarioBlock(scenarios), materialBlock(tones, Prefs.greasiness))
                .filter { it.isNotBlank() }
                .joinToString("\n\n")
        else
            ""

        val (greaseLabel, greaseRule) = Prefs.greaseLevel(Prefs.greasiness)

        return """
你在帮一个舔狗回复他女神的私信，用他的口吻写候选回复。

他的人设：
${Prefs.persona}

油腻度：${greaseLabel}（${Prefs.greasiness}/100）。${greaseRule}
...
""".trim()
    ...
    suspend fun suggest(
        ...
        // 场景匹配只看对方最近两条消息，不看你自己的
        val incoming = transcript.filter { it.role == "them" }.takeLast(2)
            .joinToString("\n") { it.text }
        val scenarios = ReplyBank.matchScenarios(incoming)

        val messages = JSONArray()
        messages.put(JSONObject().put("role", "system").put("content", systemPrompt(tones, scenarios)))
