...
object SetupGuide {
    ...
    fun items(ctx: Context, useAccessibility: Boolean): List<Item> {
        val list = mutableListOf<Item>()

        list += Item(
            key = "post",
            title = "允许发送通知",
            why = "候选回复和拦截提醒都是通过通知推给你的，不给就什么都看不到。",
        ...
        list += Item(
            key = "wxdetail",
            title = if (useAccessibility) "微信通知设置（可选）" else "微信显示消息详情",
            why = if (useAccessibility)
                "无障碍模式直接读界面，这项不影响使用。"
            else
                "不开的话通知里只有「你收到一条消息」，抓不到任何内容，功能等于废掉。",
