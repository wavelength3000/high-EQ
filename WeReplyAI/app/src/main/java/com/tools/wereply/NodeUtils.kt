...
object NodeUtils {
     ...
     * 取某个节点自己的文字。
     *
     * 只认 text 会漏掉一批 —— 微信不少气泡把内容挂在 contentDescription 上，
     * 表情消息尤其常见。两个都取。
     */
    ...
    /** 子树里还有别的节点带文字吗。有就说明当前这个是容器，内容以里层的为准。 */
    private fun hasTextChild(n: AccessibilityNodeInfo, depth: Int = 0): Boolean {
        if (depth > 6) return false
        for (i in 0 until n.childCount) {
            val c = n.getChild(i) ?: continue
            if (ownText(c) != null) return true
            if (hasTextChild(c, depth + 1)) return true
        }
        return false
    }
     ...
     * 把一段纵向区间里的文字行捞出来。
     *
     * 这里刻意不再要求 childCount == 0。微信的气泡是自绘控件，里面常常还挂着
     * 表情之类的子节点，按叶子筛会把整条消息筛掉 —— 这也是「读到 0 条」的常见原因。
    ...
    private fun collectRows(
        ...
        topLimit: Int,
        bottomLimit: Int
    ): List<Row> = findAll(root) { n ->
        !n.isEditable && ownText(n) != null && !hasTextChild(n)
    }.mapNotNull { n ->
        var text = ownText(n) ?: return@mapNotNull null
        if (text.isBlank() || text.length > 500) return@mapNotNull null

        val b = bounds(n)
        if (b.height() == 0 || b.width() == 0) return@mapNotNull null
        if (b.top < topLimit || b.bottom > bottomLimit) return@mapNotNull null
    ...
    fun extractMessages(
        ...
        // 先按标题栏/输入框之间这个窄区间读一遍；一条都没有就把上下界放开重来。
        // 微信改版之后输入框位置经常变，卡死在窄区间上等于白读。
        var rows = collectRows(root, topLimit, inputTop)
        if (rows.isEmpty()) rows = collectRows(root, 0, screenHeight)
