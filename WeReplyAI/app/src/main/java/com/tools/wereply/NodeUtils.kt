...
object NodeUtils {
    ...
    /**
     * 读出屏幕上可见的聊天记录。
     * 只在消息列表容器内部找，所以标题栏、底部工具栏、输入框都不会混进来。
     */
    fun extractMessages(root: AccessibilityNodeInfo?, screenWidth: Int): List<ChatMsg> {
        ...
        return findAll(list) { n ->
            ...
            if (text.length > 500 || text.isBlank()) return@mapNotNull null
            // 时间戳、系统提示这类短标记过滤掉
            if (text.matches(Regex("^\\d{1,2}[:：]\\d{2}$"))) return@mapNotNull null
            if (text in setOf("昨天", "今天", "星期一", "星期二", "星期三", "星期四",
                    "星期五", "星期六", "星期日", "以下为新消息")) return@mapNotNull null

            val b = bounds(n)
            ChatMsg(text, fromMe = b.centerX() > screenWidth * 0.55)
        }
    }
